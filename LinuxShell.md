...
##### 1、查看rsync版本号：rpm -q rsync 

查看centos版本号：rpm -q centos-release 

##### 2、切换服务器，可以使用-p指定端口号 -p 2222

ssh root@192.168.89.150，然后输入密码即可

跨服务器拷贝：

a、将本地文件拷贝到别的服务器

scp  **/etc/xxx/***  **root@192.168.89.150:/mysqldump**  #前一个目录是本地服务器待拷贝内容目录，后一个是指定接收文件的服务器及目录。

b、将别的服务器文件拷贝到备份目录

scp root@192.168.89.150:/mysqldump /backup

##### 3、nohup

nohup java -jar demo-1.0.1.jar >springboot.log 2>&1 &

守护进程启动java项目，并且将log日志输出打印到指定文件

nohup：表示守护进程运行，即关掉exe页面后项目也会在后台运行。>nohub.log 指定输出文件

2>&1 ：表示错误内容输出指定到正确内容输出文件中。

& 最后的这个&标识：是守护进程执行

such as： nohub ./test.sh >nohub.log 2>&1 &
...
##### 7、vi操作：https://blog.csdn.net/qq_28112129/article/details/78529848
...
- **`mkdir 目录名称`：** 增加目录。
  ...
  mkdir -p /a/b :-p的意思：如果上级目录a不存在，就先创建a在创建目录b
...
- **`mv 目录名称 新目录名称`：** 修改目录的名称（改）。注意：mv 的语法不仅可以对目录进行重命名而且也可以对各种文件，压缩包等进行 重命名的操作。mv 命令用来对文件或目录重新命名，或者将文件从一个目录移到另一个目录中。后面会介绍到 mv 命令的另一个用法。**mv a.txt b.txt c.txt -t targetdir 将多个文件或者文件夹批量移动到指定文件夹中**
